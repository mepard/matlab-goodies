function DumpHandleTree (h)
	DumpNode ([0], h, true);
	fprintf (1, '\n\n')
	DumpNode ([0], h, false);
end

function DumpNode (path, h, outline)
	if outline
		fprintf (1, '%s%s %g\n', repmat(' ', 1, 3*(length(path)-1)), get(h,'type'), h)
	else
		fprintf (1, 'Path %s: %s %g %s\n', ToString(path), get(h,'type'), h, repmat('-', 1, 80))
		get(h)
		getundoc(h)
	end
	...
	for c = 1:length(children)
		DumpNode([path c], children(c), outline)
	end
end
